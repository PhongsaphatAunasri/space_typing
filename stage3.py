    ...
    while running:
        ...
        if state == 1:
            ...
            if len(falling_words) == 0 or (state_timer > 2000 and len(falling_words) < 3):
                ...
                if available_positions:  # Ensure there's at least one position left
                    ...
                    falling_words.append(Stage3(existing_words=falling_words, speed=1 + random.random(), x=x))
                    state_timer = 0
            ...
            if waves_completed < max_wave_count:
                if state_timer < 3000 and len(falling_words) < 1:
                    ...
                    for i in range(min(4, len(possible_positions))):  # Ensure max 5 words, avoiding index error
                        falling_words.append(Stage3(existing_words=[], speed=0.5 + random.random(), x = possible_positions[i]))
                    wave_count += 1
                    waves_completed += 1
                    state_timer = 0
            else:
        ...
        #---------------------------- state 3 -----------------------------#
            ...
            if boss.current_word:
                ...
                remaining_surface = font.render(remaining_part, True, config.DARKGREEN)
