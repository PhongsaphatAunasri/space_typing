...
from FallingWord import Stage3
...
def game_over_menu(player_score):
    ...
    while running:
        ...
        draw_text("Stage 3", pygame.font.Font("assets/Prototype.ttf", 100), config.WHITE, config.WIDTH // 2, config.HEIGHT // 3)
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    if current_selection == 0:  # Restart option
                        return adventure_s3()  # Restart the game with initial health
...
def adventure_s3():
    ...
    state = 1
    state_timer = 0
    boss = Boss3(x=config.WIDTH // 2 - 100, y=100, health=100)
    falling_words = []
    remembered_words = []
    ...
    while running:
        ...
        if state == 1:
            ...
            if len(falling_words) == 0 or (state_timer > 2000 and len(falling_words) < 3):
                falling_words.append(Stage3(existing_words=[], speed=1 + random.random()))
                state_timer = 0
        ...
        #---------------------------- state 2 -----------------------------#
            ...
            if waves_completed < max_wave_count:
                if state_timer < 3000 and len(falling_words) < 1:
                    for _ in range(3):
                        falling_words.append(Stage3(existing_words=[], speed=0.5 + random.random()))
                    wave_count += 1
                    waves_completed += 1
                    state_timer = 0
            else:
                if len(falling_words) == 0:
                    state = 3
                    boss = Boss3(x=config.WIDTH // 2 , y=config.HEIGHT // 4 , health=boss_health, word_file="assets/csv/boss3.csv")
                    state_timer = 0
        ...
        #---------------------------- state 3 -----------------------------#
            ...
            if boss.current_word:
                ...
                # Render both parts
                correct_surface = font.render(correct_part, True, config.BLACK)
