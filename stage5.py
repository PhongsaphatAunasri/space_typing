...
from FallingWord import Stage5
...
def game_over_menu(player_score):
    ...
    while running:
        ...
        draw_text("Stage 5", pygame.font.Font("assets/Prototype.ttf", 100), config.WHITE, config.WIDTH // 2, config.HEIGHT // 3)
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    if current_selection == 0:  # Restart option
                        return adventure_s5()  # Restart the game with initial health
...
def adventure_s5():
    ...
    state = 1
    state_timer = 0
    boss = Boss5(x=config.WIDTH // 2 - 100, y=100, health=100)
    falling_words = []
    remembered_words = []
    ...
    while running:
        ...
        if state == 1:
            ...
            if len(falling_words) == 0 or (state_timer > 2000 and len(falling_words) < 3):
                ...
                if available_positions:  # Ensure there's at least one position left
                    ...
                    falling_words.append(Stage5(existing_words=falling_words, speed=1 + random.random(), x=x))
                    state_timer = 0
            ...
            if waves_completed < max_wave_count:
                if state_timer < 3000 and len(falling_words) < 1:
                    ...
                    for i in range(min(4, len(possible_positions))):  # Ensure max 5 words, avoiding index error
                        falling_words.append(Stage5(existing_words=[], speed=0.5 + random.random(), x = possible_positions[i]))
                    wave_count += 1
                    waves_completed += 1
                    state_timer = 0
            else:
                if len(falling_words) == 0:
                    state = 3
                    boss = Boss5(x=config.WIDTH // 2 , y=config.HEIGHT // 4 , health=boss_health, word_file="assets/csv/boss5.csv")
                    state_timer = 0
        ...
        #---------------------------- state 3 -----------------------------#
            ...
            if boss.current_word:
                ...
                # Render both parts
                correct_surface = font.render(correct_part, True, config.PINK)
                remaining_surface = font.render(remaining_part, True, config.DARKPURPLE)
