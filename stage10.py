...
from FallingWord import Stage10
from Boss import Boss10
...
def game_over_menu(player_score):
    ...
    while running:
        ...
        draw_text("Stage 10", pygame.font.Font("assets/Prototype.ttf", 100), config.WHITE, config.WIDTH // 2, config.HEIGHT // 3)
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    if current_selection == 0:  # Restart option
                        return adventure_s10()  # Restart the game with initial health
...
def adventure_s10():
    ...
    state = 1
    state_timer = 0
    boss = Boss10(x=config.WIDTH // 2 - 100, y=100, health=100)
    falling_words = []
    remembered_words = []
    ...
    while running:
        ...
        #---------------------------- state 2 -----------------------------#
            ...
            if waves_completed < max_wave_count:
                ...
                if len(falling_words) == 0:
                    state = 3
                    boss = Boss10(x=config.WIDTH // 2 , y=config.HEIGHT // 4 , health=boss_health, word_file="assets/csv/boss10.csv")
                    state_timer = 0
        ...
        #---------------------------- state 3 -----------------------------#
            ...
            if boss.current_word:
                ...
                # Render both parts
                correct_surface = font.render(correct_part, True, config.PURPLE)
